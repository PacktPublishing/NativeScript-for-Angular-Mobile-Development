...
export class MixerService {
    ...
    for (let comp of compositions) {
      ...
      for (let track of comp.tracks) {
        ...
        for (let key in track) {
          ...
          if (!key.includes('_') && !key.includes('$') && key != 'model' && key != 'durationDisplay') {
  ...
  public hydrate(compositions: Array<IComposition>) {
    for (let c = 0; c < compositions.length; c++) {
      ...
      if (comp.tracks && comp.tracks.length) {
  ...
  public deleteFiles(paths: Array<string>) {
    if (paths && paths.length) {
      this.log.debug('deleting files:', paths);
      let path = paths[0];
      const removeFile = () => {
        paths.splice(0, 1);
        if (paths.length) {
          ...
          this.log.debug('deleted all track files.');
        }
      }
      deleteFile(path).then(removeFile.bind(this), removeFile.bind(this));
    }
  }
