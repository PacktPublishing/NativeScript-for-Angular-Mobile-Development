...
export class RecorderService {
  ...
  private _handleSaved() {
    ...
    if (this._originalFilepath) {
      ...
      let file = File.fromPath(this._originalFilepath);
      try {
        file.remove().then((result) => {
          this.log.debug('Removed older recording file:', result);
        }, (err) => {
          this.log.debug('Error removing older recording:', err);
        })
      } catch (err) {
        this.log.debug('Could not remove older recording:', err);
      }
    }
    this.model.finish();
  }
